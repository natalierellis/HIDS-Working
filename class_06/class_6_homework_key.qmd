...
```{sql connection=con}
WITH Smokers AS (
      -- DISTINCT ON is a Postgres only feature, equivilent to using row_number() and taking only the first row
      SELECT DISTINCT ON (patient) patient, date, 1 AS smoker
      FROM observations
      WHERE code = '72166-2' AND value = 'Smokes tobacco daily (finding)'
      AND date < '2020-09-24'
      ORDER BY patient, date DESC
    )
    -- This is a common technique which we haven't discussed in class
...
WITH active_careplans AS (
    ...
    FROM careplans
    WHERE
    description ilike '%asthma%'
    AND stop IS NULL -- its active
...
SELECT count(DISTINCT AsthmaCohort.patient) patients_with_both_medications
  ...
  INNER JOIN medications medications1 ON medications1.patient = AsthmaCohort.patient
    ...
    AND (medications1.stop BETWEEN '2019-09-24' AND '2020-09-24' OR (medications1.start >= '2019-09-24' AND medications1.stop IS NULL) ) 
  INNER JOIN medications medications2 ON medications2.patient = AsthmaCohort.patient
    AND medications2.code = '2123111'
    AND (medications2.stop BETWEEN '2019-09-24' AND '2020-09-24' OR (medications2.start >= '2019-09-24' AND medications2.stop IS NULL) )     
```
